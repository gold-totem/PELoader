...
bool PELdr::InProcLoader::loadPE(unsigned char* peBuffer) {
    ...
    for (DWORD sectionIndex{ 0 }; sectionIndex < numberOfSections; ++sectionIndex) {
        ...
        if (!VirtualProtect(reinterpret_cast<void*>(baseAddress + sectionHeader->VirtualAddress), sectionHeader->Misc.VirtualSize, sectionProt, &oldFlags)) {
...
bool PELdr::InProcLoader::callEntry() {
...
bool  PELdr::InProcLoader::callExport(std::string_view funcName) {
    ...
    for (UINT32 i = 0; i < exportTable->NumberOfNames; i++) {
        ...
        char* functionName = (char*)(baseAddress + nameRVA);
        if (std::strcmp(funcName.data(), functionName) == 0) {

            PUINT16 ordinalTable = reinterpret_cast<PUINT16>(baseAddress + exportTable->AddressOfNameOrdinals);
            DWORD* exportAddressTable = reinterpret_cast<DWORD*>(baseAddress + exportTable->AddressOfFunctions);
            DWORD ordinal = ordinalTable[i];
            uintptr_t offset = static_cast<uintptr_t>(exportAddressTable[ordinal]);

            auto func{ reinterpret_cast<LPTHREAD_START_ROUTINE>(baseAddress + offset) };
            func(NULL);
            return true;
        }
    }
