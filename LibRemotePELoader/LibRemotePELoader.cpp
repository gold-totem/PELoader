...
namespace {
    ...
    bool injectDLL(HANDLE hProc, std::string_view dllName) {
        ...
        if (!baseAddress) {
            std::cerr << "VirtualAllocEx failed (DLL inject): " << GetLastError() << '\n';
            return false;
        }
        ...
        if (!WriteProcessMemory(hProc, baseAddress, dllName.data(), sizeOfPath, &numBytesWritten)) {
            std::cerr << "WriteProcessMemory failed (DLL inject): " << GetLastError() << '\n';
            VirtualFreeEx(hProc, baseAddress, 0, MEM_RELEASE);
            return false;
        }

        if (numBytesWritten < sizeOfPath) {
            std::cerr << "Error: Partial write(DLL inject)\n";
            VirtualFreeEx(hProc, baseAddress, 0, MEM_RELEASE);
            return false;
        }
        ...
        if (!hThread) {
            std::cerr << "CreateRemoteThread failed (DLL inject) : " << GetLastError() << '\n';
