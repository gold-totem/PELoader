...
bool LoadDLL::DllLoader::loadDLL(HANDLE hProc, unsigned char* peBuffer, std::string_view functionName) {
    if (!hProc) {
    ...
    PIMAGE_NT_HEADERS pNTHeader = reinterpret_cast<PIMAGE_NT_HEADERS>(peBuffer + dosHeader->e_lfanew);
    ...
    PVOID buffer = VirtualAlloc(NULL, pNTHeader->OptionalHeader.SizeOfImage, MEM_COMMIT, PAGE_READWRITE);
    //buffer.resize(1024);
    ...
    uintptr_t delta = reinterpret_cast<uintptr_t>(baseAddressAlloc) - pNTHeader->OptionalHeader.ImageBase;
    ...
    unsigned char* baseAddress{ reinterpret_cast<unsigned char*>(buffer) };

    PIMAGE_EXPORT_DIRECTORY pExportTable = reinterpret_cast<PIMAGE_EXPORT_DIRECTORY>(baseAddress + pNTHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress);
    PUINT16 ordinalTable = reinterpret_cast<PUINT16>(baseAddress + pExportTable->AddressOfNameOrdinals);
    PUINT32 nameArray{ reinterpret_cast<PUINT32>(baseAddress + pExportTable->AddressOfNames) };
    DWORD* pExportAddressTable = reinterpret_cast<DWORD*>(baseAddress + pExportTable->AddressOfFunctions);
    uintptr_t functionAddress{ 0 };

    for (UINT32 index = 0; index < pExportTable->NumberOfNames; index++) {

        UINT32 nameRVA = nameArray[index];
        char* funcName = (char*)(baseAddress + nameRVA);
    ...
    for (int i = 0; i < pNTHeader->OptionalHeader.SizeOfImage;i++ ) {
        std::cout << (char)baseAddress[i];
    ...
    HANDLE hThread = CreateRemoteThread(hProc, NULL, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>( (unsigned char*)baseAddressAlloc + functionAddress), NULL, 0, NULL);
    if (!hThread) {
